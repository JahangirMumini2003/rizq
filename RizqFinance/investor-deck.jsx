    ...
    <section style={{ background: "var(--bg-deep)", color: "var(--inverse)", position: "relative", overflow: "hidden" }}>
      ...
      <div className="container" style={{ position: "relative", zIndex: 1 }}>
        ...
        <PathBlock
          ...
          tranches={[
            { tag: "Сразу", pct: "53%", amt: "$160K", title: "Уставный капитал МФО", body: "Обязательный минимум для лицензии МФО от ЦБ Узбекистана. Без этого нельзя выдавать средства от своего имени." },
            { tag: "Транш 1", pct: "23%", amt: "$70K", title: "До 30K сотрудников", body: "Tech 60% · Sales 30% · Юр 10%. Платформа, B2B-продажи, юридическое оформление пилотов и договоров." },
            { tag: "Транш 2", pct: "23%", amt: "$70K", title: "После 30K сотрудников", body: "Те же пропорции 60/30/10. Расширение команды, новые регионы, более глубокая интеграция с ERP." },
          ]}
          gets={[
...
/* PathBlock — full path layout (hero + rationale + tranches + what investor gets) */
function PathBlock({ tag, tagSub, subtitle, amount, equity, chips, rationale, tranches, gets, accent }) {
    ...
    <div style={{
      ...
      <div style={{ position: "relative", marginTop: 40 }}>
        <h3 style={{ fontSize: 22, color: "var(--inverse)", letterSpacing: "-0.02em" }}>
          <span style={{ fontFamily: "var(--f-mono)", fontSize: 11, color: isBrand ? "var(--brand)" : "var(--inverse-2)", letterSpacing: "0.14em", textTransform: "uppercase", display: "block", fontWeight: 700 }}>Аллокация</span>
          <span style={{ marginTop: 4, display: "block" }}>Структура {amount}</span>
        </h3>
        <div style={{
          marginTop: 18,
          display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12,
        }} className="pb-tranches">
          {tranches.map((tr, i) => {
            const primary = isBrand && i === 0;
            return (
              <div key={i} style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <span style={{
                    ...
                    background: primary ? "var(--brand-ink)" : "oklch(0.22 0.05 158)",
                    color: "var(--brand)",
                    border: primary ? "none" : "1px solid oklch(0.32 0.05 158)",
                  }}>{tr.tag}</span>
                  <span style={{
                    fontFamily: "var(--f-mono)", fontSize: 11, fontWeight: 800,
                    letterSpacing: "0.08em",
                    opacity: 0.7,
                    color: primary ? "var(--brand-ink)" : "var(--inverse-2)",
                  }}>{tr.pct}</span>
                </div>
                <div style={{ fontSize: 13, fontWeight: 700, color: primary ? "inherit" : "var(--inverse)" }}>{tr.title}</div>
                <div className="pb-tr-amt" style={{ fontSize: 44, fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1, color: primary ? "inherit" : "var(--inverse)" }}>{tr.amt}</div>
                <p style={{
                  marginTop: "auto", fontSize: 12.5, lineHeight: 1.55,
                  color: primary ? "oklch(0.18 0.05 155 / 0.75)" : "var(--inverse-2)",
                }}>{tr.body}</p>
              </div>
            );
          })}
      ...
      <style>{`
        @media (max-width: 1000px) {
          ...
          .pb-tranches { grid-template-columns: 1fr !important; }
        ...
        @media (max-width: 560px) {
          ...
          .pb-tr-amt { font-size: 36px !important; }
